...
func (s *SupplyChainContract) UpdateProduct(ctx contractapi.TransactionContextInterface, id, newStatus, newOwner, newDescription, newCategory string) error {
    ...
    // 🚨 Fix: only update owner if a new owner is provided
    if newOwner != "" {
...
// putProduct inserts or updates a product in the ledger
...
// GetAllProducts retrieves all products from the ledger
